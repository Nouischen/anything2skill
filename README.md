...
- **融合防通膨**：新內容進來先跟你既有的 skill 庫比對——同一位老師的方法越講越完整就**併進去深化**；**同一套框架套在不同產業（咖啡四象限 vs 茶葉四象限）會抽成「一個框架＋多領域範例」而不是堆一堆近重複**；不同派系的方法並存分流、絕不攪拌；真有衝突（A 派台上報價、B 派台下談）會停下來讓你選。你的 skill 庫用一年還是乾淨的。
